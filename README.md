...
-   ダミーデータ（各ユーザー）でのログイン、register後のログインの際にメール確認が完了している必要があります。ダミーデータでのログインの際はメールの再送が必要となるため、再度送信するボタンをクリックしてください。また、メールの送受信のテストは、他に予約リマインダーメールでも使用されます。メール送受信テストにはMailhogを使用します。インストール方法は使用する機器により変わるため各自でご確認ください。使用するために必要なインストール・設定後に<http://localhost/8025>にアクセスすることでメール確認を行うことができます。メール確認後、後ログインが可能となります。
